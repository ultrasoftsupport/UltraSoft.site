-- =========================================================================
-- 🚀 MIGRATION V96: DEFENSIVE WORKER_ID VALIDATION IN PROCESS_ORDER_TRANSACTION
-- 💡 Description: Prevents foreign key violation error 23503 (orders_worker_id_fkey)
--    when an order is created with a stale, deleted, or orphaned worker UUID.
-- =========================================================================
...
BEGIN
    ...
    -- 🛡️ التحقق الدفاعي: إذا كان worker_id غير موجود في system_users، يتم تصفيره لمنع كسر Foreign Key (23503)
    IF v_worker_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM public.system_users WHERE id = v_worker_id) THEN
        v_worker_id := NULL;
    END IF;
