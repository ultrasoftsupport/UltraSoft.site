-- Migration v63: Ensure standard plans read rules dynamically from subscription_plan_limits
-- Clean legacy hardcoded bulk_edit_max_cap overrides from non-custom subscriptions

-- 1. Remove hardcoded bulk_edit_max_cap & plan rules from custom_limits for non-custom subscriptions
UPDATE public.subscriptions
SET custom_limits = custom_limits - 'bulk_edit_max_cap' - 'bulk_edit_pricing_mode' - 'bulk_edit_per_op_cost' - 'bulk_edit_per_item_cost' - 'excel_upload_colors_cost' - 'excel_upload_items_cost' - 'excel_upload_stock_cost'
WHERE plan != 'custom' AND custom_limits IS NOT NULL;

-- 2. Re-create get_tenant_credit_rules RPC
...
-- 3. Re-create deduct_excel_credits RPC
