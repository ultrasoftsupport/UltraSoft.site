-- =========================================================================
-- 🚀 MIGRATION V21: TENANT-SCOPED SETTINGS & PER-TENANT SEQUENTIAL INVOICES
-- 📦 Project: UltraSoft (ألترا سوفت)
-- 📅 Date: 2026-08-02
-- 💡 Description: Fixes home_settings primary key to (tenant_id, setting_key)
--    and updates process_order_transaction to generate invoice numbers sequentially
--    starting from 1 per tenant independently.
-- =========================================================================

-- 1. Ensure composite primary key on home_settings for per-tenant setting isolation
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'home_settings' AND constraint_type = 'PRIMARY KEY'
    ) THEN
        ALTER TABLE public.home_settings DROP CONSTRAINT home_settings_pkey;
    END IF;
    
    -- Ensure tenant_id is NOT NULL for default rows if any
    UPDATE public.home_settings SET tenant_id = '00000000-0000-0000-0000-000000000001' WHERE tenant_id IS NULL;
    
    ALTER TABLE public.home_settings ADD CONSTRAINT home_settings_pkey PRIMARY KEY (tenant_id, setting_key);
EXCEPTION
    WHEN OTHERS THEN
        NULL; -- Handle gracefully if already configured
END $$;

-- 2. Update process_order_transaction to calculate per-tenant sequential invoice numbers starting from 1
...
BEGIN
    -- Extract tenant_id or fallback to current_tenant_id()
    v_tenant_id := COALESCE((p_order_data->>'tenant_id')::uuid, public.current_tenant_id());
    ...
    ELSE
        ...
        -- 🎯 Generate sequential invoice number isolated per tenant (starts from 1 per tenant)
