-- =========================================================================
-- 🌟 MIGRATION V34: PER-TENANT TELEGRAM BOT & NOTIFICATION ISOLATION 🌟
-- =========================================================================
-- تاريخ الإنشاء: 2026-08-03
-- الإصدار: v34.0
-- الوصف: 
-- 1. التأكد من أن جميع استعلامات التليجرام وإرسال التنبيهات تقرأ إعدادات البوت والجروب المحددة لكل مصنع بناءً على tenant_id الخاص به حصرياً.
-- =========================================================================

-- 1. تحديث دالة أتمتة إشعارات تغييرات الأوردرات والتنبيهات المباشرة مع عزل المصانع
...
BEGIN
    ...
    IF (TG_OP = 'INSERT') THEN
        ...
        VALUES (notification_type, notification_title, notification_body, jsonb_build_object('order_id', NEW.id, 'customer_name', NEW.customer_name, 'total_price', NEW.total_price), v_tenant_id);
    ...
    ELSIF (TG_OP = 'UPDATE') THEN
        IF (NEW.worker_id IS DISTINCT FROM OLD.worker_id AND NEW.worker_id IS NOT NULL) THEN
            ...
            VALUES (notification_type, notification_title, notification_body, jsonb_build_object('order_id', NEW.id), target_user_id, v_tenant_id);
        END IF;
        ...
        IF (NEW.status IS DISTINCT FROM OLD.status OR NEW.total_price IS DISTINCT FROM OLD.total_price OR NEW.is_locked IS DISTINCT FROM OLD.is_locked) THEN
            ...
            INSERT INTO public.system_notifications (type, title, body, metadata, tenant_id)
            VALUES (notification_type, notification_title, notification_body, jsonb_build_object('order_id', NEW.id, 'status', NEW.status, 'total_price', NEW.total_price), v_tenant_id);
        END IF;
    END IF;
...
-- 2. تحديث معالجة طابور تنبيهات المخزون مجمعة مع إرسال الإشعار لـ Telegram البوت التابع للمصنع المحدد
...
BEGIN
    ...
    -- تحديد المصنع المعني بالسجلات في هذا المعاملة
    SELECT tenant_id INTO v_tenant_id FROM public.inventory_notification_queue WHERE transaction_id = v_tx_id LIMIT 1;

    -- قراءة إعدادات البوت والجروب التابعة لهذا المصنع تحديداً
    SELECT setting_value INTO v_bot_token FROM public.home_settings WHERE tenant_id = v_tenant_id AND setting_key = 'telegram_bot_token';
    SELECT setting_value INTO v_is_tg_enabled FROM public.home_settings WHERE tenant_id = v_tenant_id AND setting_key = 'telegram_enabled';
    SELECT setting_value INTO v_is_stock_enabled FROM public.home_settings WHERE tenant_id = v_tenant_id AND setting_key = 'telegram_stock_enabled';
    
    -- Fallback للمصنع الافتراضي إن لم توجد
    IF v_bot_token IS NULL OR v_bot_token = '' THEN
        SELECT setting_value INTO v_bot_token FROM public.home_settings WHERE setting_key = 'telegram_bot_token' LIMIT 1;
        SELECT setting_value INTO v_is_tg_enabled FROM public.home_settings WHERE setting_key = 'telegram_enabled' LIMIT 1;
        SELECT setting_value INTO v_is_stock_enabled FROM public.home_settings WHERE setting_key = 'telegram_stock_enabled' LIMIT 1;
    END IF;

    IF (COALESCE(v_is_tg_enabled, 'true') = 'true' AND COALESCE(v_is_stock_enabled, 'true') = 'true' AND v_bot_token IS NOT NULL AND v_bot_token <> '') THEN
        SELECT setting_value INTO v_chat_id FROM public.home_settings WHERE tenant_id = v_tenant_id AND setting_key = 'telegram_stock_chat_id';
        IF (v_chat_id IS NULL OR v_chat_id = '') THEN
            SELECT setting_value INTO v_chat_id FROM public.home_settings WHERE tenant_id = v_tenant_id AND setting_key = 'telegram_chat_id';
        END IF;

        IF (v_chat_id IS NULL OR v_chat_id = '') THEN
            SELECT setting_value INTO v_chat_id FROM public.home_settings WHERE setting_key = 'telegram_stock_chat_id' LIMIT 1;
            IF (v_chat_id IS NULL OR v_chat_id = '') THEN
                SELECT setting_value INTO v_chat_id FROM public.home_settings WHERE setting_key = 'telegram_chat_id' LIMIT 1;
            END IF;
        END IF;
